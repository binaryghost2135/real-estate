...
export default function HomePage() {
    ...
    const [allProperties, setAllProperties] = useState<Property[]>(() => {
        if (typeof window !== 'undefined') {
            const savedProperties = localStorage.getItem(LOCAL_STORAGE_PROPERTIES_KEY);
            if (savedProperties) {
                try {
    ...
    const [likedPropertyIds, setLikedPropertyIds] = useState<string[]>(() => {
        if (typeof window !== 'undefined') {
            const savedLikedIds = localStorage.getItem(LOCAL_STORAGE_LIKED_IDS_KEY);
            if (savedLikedIds) {
                try {
                    return JSON.parse(savedLikedIds);
                } catch (e) {
    ...
    const [isLoading, setIsLoading] = useState(false); // Kept for potential future use with async ops
        ...
        if (typeof window !== 'undefined') {
            try {
                ...
                toast({
                    ...
                    description: "Could not save property data to local storage. This might be due to size limits (e.g., too many large images). Some changes might not persist across reloads if the limit is exceeded.",
                    variant: "destructive",
                    duration: 10000, 
                });
            }
    ...
    const handleLike = (propertyId: string) => {
        setLikedPropertyIds(prev => {
            ...
            toast({ title: newLikedIds.includes(propertyId) ? "Added to Favorites" : "Removed from Favorites", variant: "default" });
            return newLikedIds;
        });
    };
